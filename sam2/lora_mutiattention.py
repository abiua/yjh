...
class LoRAWithAttn(nn.Module):
    def __init__(self, dim, r=16, nhead=4, dropout=0.1):
        ...
        self.attn = nn.MultiheadAttention(embed_dim=r, num_heads=nhead, dropout=dropout)
        self.norm = nn.LayerNorm(r)
        self.drop = nn.Dropout(dropout)

    def _attn3d(self, h: torch.Tensor) -> torch.Tensor:
        """
        h: [B, N, r] → (transpose) → [N, B, r] → attn → [N, B, r] → (transpose back) → [B, N, r]
        """
        h_t = h.transpose(0, 1)               # [N, B, r]
        h2_t, _ = self.attn(h_t, h_t, h_t)    # [N, B, r]
        h2 = h2_t.transpose(0, 1)             # [B, N, r]
        return self.norm(h + self.drop(h2))   # 残差 + LayerNorm
